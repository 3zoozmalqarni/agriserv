...
export default function LabProfileSettings() {
  const { user } = useAuth();
  ...
  const handleImageSave = async () => {
    ...
    try {
      await localDB.updateUser(user.id, { profile_image: selectedImage });
  ...
  const handleImageRemove = async () => {
    ...
    try {
      await localDB.updateUser(user.id, { profile_image: null });
