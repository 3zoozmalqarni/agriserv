...
import { getCommonPrintStyles, generatePrintHeader, generatePrintFooter } from '../../../lib/printUtils';
...
export default function PrintResults({}: PrintResultsProps) {
  ...
  const getResultLabel = (result: any) => {
    // إذا كان result نص فقط (للتوافق مع الكود القديم)
    if (typeof result === 'string') {
      if (result === 'كفاءة تحصين' || result === 'vaccination_efficacy' || result === 'اختبار كفاءة تحصين') {
        return 'كفاءة تحصين';
      }
      if (result === 'إيجابي بعد التأكيد') return 'إيجابي بعد التأكيد';
      if (result === 'سلبي بعد التأكيد') return 'سلبي بعد التأكيد';
      if (result === 'positive') return 'إيجابي';
      if (result === 'negative') return 'سلبي';
      return result;
    }
    ...
    if (result.test_result === 'negative') {
      return 'سلبي';
    }

    return result.test_result || 'غير محدد';
  };

  const handlePrintProcedure = (procedure: any) => {
    const procedureSamples = procedure.samples || [];
    const samplesWithResults = procedureSamples.map((sample: any) => {
  ...
  const handlePrint = () => {
    ...
    const titleText = 'نتيجة فحص مخبري';
    thead.innerHTML = `
      <tr>
        <th style="padding: 0;">
          <div style="text-align: center; padding: 6px 0; margin-bottom: 6px; border-bottom: 2px solid #008a40;">
            ...
            <h2 style="color: #00a651 !important; font-size: 13px; font-weight: 600; margin: 0;">${titleText}</h2>
            <div style="border: 1px solid #e5e7eb; border-radius: 4px; padding: 4px 0; margin-top: 4px; display: flex; justify-content: space-around; align-items: center; flex-wrap: wrap; gap: 8px;">
              <div style="font-size: 7px; display: flex; align-items: center; gap: 3px;"><span style="font-weight: 700; color: #6b7280;">رقم الإجراء:</span> <span style="font-weight: 600; color: #111827;">${printContent.procedure.internal_procedure_number || 'غير محدد'}</span></div>
              ...
              <div style="font-size: 7px; display: flex; align-items: center; gap: 3px;"><span style="font-weight: 700; color: #6b7280;">التاريخ:</span> <span style="font-weight: 600; color: #111827;">${printContent.procedure.reception_date}</span></div>
    ...
    const oldStyles = document.getElementById('dynamic-print-styles');
  ...
  const generatePrintHTML = () => {
    ...
    const content = printContent.type === 'procedure'
      ? generateProcedureHTML()
      : generateSampleHTML();
    ...
    return `
      ...
      <head>
        ...
        <title>تقرير نتائج الفحوصات</title>
        ${getCommonPrintStyles()}
      </head>
      <body>
        <div class="print-container">
          ${generatePrintHeader('تقرير نتائج الفحوصات المخبرية')}
          ${content}
          ${generatePrintFooter()}
        </div>
      </body>
  ...
  const generateProcedureHTML = () => {
    ...
    const procedureInfoHTML = `
        <div class="section" style="margin-bottom: 6px; padding: 0; border: none; box-shadow: none;">
          <div class="section-title" style="margin-bottom: 4px; padding-bottom: 2px; font-size: 10pt;">معلومات الإجراء</div>
          <div class="info-grid" style="grid-template-columns: repeat(3, 1fr); gap: 2px; row-gap: 1px;">
            <div class="info-item" style="padding: 0; border: none; background: transparent;">
              <span class="info-label" style="font-size: 7pt;">رقم الإجراء المخبري:</span>
              <span class="info-value" style="font-size: 7.5pt; font-weight: 600;">${proc.internal_procedure_number || '-'}</span>
            </div>
            <div class="info-item" style="padding: 0; border: none; background: transparent;">
              <span class="info-label" style="font-size: 7pt;">رقم الإجراء الخارجي:</span>
            ...
            <div class="info-item" style="padding: 0; border: none; background: transparent;">
              <span class="info-label" style="font-size: 7pt;">تاريخ الإجراء الخارجي:</span>
    ...
    return `
      <div class="page">
        ...
        <div class="section">
          ...
          if (!result) return `
            <div class="sample-card" style="padding: 10px; margin-bottom: 10px;">
              <div style="font-weight: bold; margin-bottom: 8px; font-size: 10pt; border-bottom: 1px solid #e5e7eb; padding-bottom: 6px;">العينة رقم ${index + 1} - ${sample.sample_number || '-'}</div>
              <div style="color: #6b7280; text-align: center; padding: 8px; font-size: 9pt;">لم يتم فحص هذه العينة بعد</div>
            </div>
          `;
          return `
            <div class="sample-card" style="padding: 0; margin-bottom: 4px; background: transparent; border: none; border-radius: 0; box-shadow: none;">
              <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 2px; padding-bottom: 2px; border-bottom: 1px solid #e5e7eb;">
                <span class="badge ${
                  (result.test_result === 'إيجابي بعد التأكيد' || result.test_result === 'positive') ? 'badge-positive' :
                  (result.test_result === 'سلبي بعد التأكيد' || result.test_result === 'negative') ? 'badge-negative' :
                  'badge-vaccination'
      ...
      ${totalPages > 1 ? Array.from({ length: totalPages - 1 }, (_, pageIndex) => {
        ...
        return `
          ...
          <div class="page">
            ${generatePrintHeader('تقرير نتائج الفحوصات المخبرية', true)}
              ...
              ${pageSamples.map((item: any, idx: number) => {
                ...
                if (!result) return `
                  <div class="sample-card">
                    <div style="font-weight: bold; margin-bottom: 10px;">العينة رقم ${globalIndex + 1} - ${sample.sample_number || '-'}</div>
                    <div style="color: #6b7280; text-align: center; padding: 10px;">لم يتم فحص هذه العينة بعد</div>
                  </div>
                `;
                ...
                return `
                  <div class="sample-card">
                    ...
                    <div class="info-grid">
                      ...
                      ${result.is_vaccination_efficiency_test && result.vaccination_efficiency_percentage ? `
                      <div class="info-item" style="background: #eff6ff; border: 1px solid #93c5fd; padding: 8px; border-radius: 4px;">
                        <span class="info-label" style="color: #1e40af; font-weight: bold;">نسبة كفاءة التحصين:</span>
                        <span class="info-value" style="color: #1e3a8a; font-weight: bold;">${result.vaccination_efficiency_percentage}%</span>
                      </div>
                      ` : ''}
  ...
  const generateSampleHTML = () => {
    ...
    return `
      <div class="section">
        ...
        <div class="info-grid">
          <div class="info-item">
            <span class="info-label">رقم الإجراء المخبري:</span>
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 p-8">
          <PageHeader
            icon={Printer}
            title="طباعة النتائج"
            subtitle="طباعة وتصدير نتائج الفحوصات المخبرية"
          />
                    ...
                    <div className="flex-1 flex items-center justify-between">
                      ...
                      <div className="text-center">
                        ...
                        <div className="flex items-center justify-center gap-2 text-xs text-gray-600">
                          <span>{procedure.external_procedure_number || '-'}</span>
                          <span>•</span>
                    ...
                    <div className="border-t border-gray-200 p-6 bg-gray-50">
                      <div className="mb-6 p-4 bg-white rounded-lg shadow-sm">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm text-right">
                          ...
                          <div>
                            <span className="font-semibold text-gray-700">تاريخ الإجراء الخارجي: </span>
                          ...
                          return (
                            <div key={sample.id} className="bg-white rounded-lg shadow-md border border-gray-200 p-4 hover:shadow-lg transition-shadow flex flex-col h-full" dir="ltr">
                              ...
                              <div className="space-y-2 text-xs flex-1 mt-3">
                                ...
                                <div className="flex justify-between">
                                  <span className="text-secondary-600 font-bold text-sm">{sample.sample_count}</span>
                                  ...
                                  <div className="space-y-1.5 text-xs">
                                    ...
                                    {result.is_vaccination_efficiency_test && result.vaccination_efficiency_percentage && (
                                      <div className="flex justify-between bg-blue-50 -mx-2 px-2 py-1 rounded">
                                        <span className="text-blue-600 font-bold">{result.vaccination_efficiency_percentage}%</span>
          ...
          {filteredProcedures.length === 0 && !proceduresLoading && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500 text-lg">
                {searchQuery ? 'لا توجد إجراءات مطابقة لعملية البحث' : 'لا توجد إجراءات لطباعة النتائج'}
              </p>
            </div>
          )}
        </div>
            ...
            <div className="flex-1 overflow-y-auto p-8">
              <div className="max-w-4xl mx-auto print-modal-content">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8" dir="rtl">
                  <div className="bg-[#003361]/5 p-4 rounded-lg no-break" dir="rtl">
                    <h3 className="font-bold text-[#003361] mb-3 text-right">معلومات أساسية</h3>
                    <div className="space-y-2 text-right" dir="rtl">
                      <div dir="rtl"><span className="font-semibold">رقم الإجراء المخبري:</span> {printContent.procedure.internal_procedure_number || 'غير محدد'}</div>
                      <div dir="rtl"><span className="font-semibold">رقم الإجراء الخارجي:</span> {printContent.procedure.external_procedure_number || 'غير محدد'}</div>
                      <div dir="rtl"><span className="font-semibold">اسم العميل:</span> {printContent.procedure.client_name}</div>
                      <div dir="rtl"><span className="font-semibold">تاريخ الإجراء المخبري:</span> {printContent.procedure.reception_date}</div>
                      <div dir="rtl"><span className="font-semibold">بلد المنشأ:</span> {printContent.procedure.country_port || 'غير محدد'}</div>
                    </div>
                  </div>

                  <div className="bg-[#00a651]/5 p-4 rounded-lg no-break" dir="rtl">
                    <h3 className="font-bold text-[#00a651] mb-3 text-right">معلومات إضافية</h3>
                    <div className="space-y-2 text-right" dir="rtl">
                      <div dir="rtl"><span className="font-semibold">تاريخ الإجراء الخارجي:</span> {printContent.procedure.external_procedure_date || 'غير محدد'}</div>
                      <div dir="rtl"><span className="font-semibold">مستلم العينة:</span> {printContent.procedure.receiver_name}</div>
                      <div dir="rtl"><span className="font-semibold">مصدر العينة:</span> {printContent.procedure.sample_origin || 'غير محدد'}</div>
                      <div dir="rtl"><span className="font-semibold">السجل المدني:</span> {printContent.procedure.civil_record || 'غير محدد'}</div>
                    </div>
                  </div>
                </div>
