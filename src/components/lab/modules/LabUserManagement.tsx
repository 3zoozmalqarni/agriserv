...
const userSchema = z.object({
  ...
  role: z.enum(['program_manager', 'quarantine_general_supervisor', 'lab_manager', 'lab_section_supervisor', 'lab_specialist']),
});
...
const roleLabels = {
  ...
  lab_manager: 'مدير قسم المختبر',
  lab_section_supervisor: 'مشرف مجموعة',
...
export default function UserManagement({ searchData }: UserManagementProps) {
  ...
  const { users, loading, addUser, updateUser, deleteUser, toggleUserStatus } = useUsers();

  // عند وجود بيانات من البحث
  useEffect(() => {
    if (searchData?.type === 'user' && searchData.data && !searchProcessedRef.current) {
      searchProcessedRef.current = true;
      const user = users.find(u => u.id === searchData.data.id);
      if (user) {
        setEditingUser(user);
        setShowAddForm(true);
      }
  ...
  const handleDelete = (user: any) => {
    ...
    if (user.role === 'program_manager' || user.role === 'quarantine_general_supervisor') {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="text-right">
              ...
              >
                ...
                {(!users.some(u => u.role === 'lab_manager') || (editingUser && editingUser.role === 'lab_manager')) && (
                  <option value="lab_manager">مدير قسم المختبر</option>
                )}
                <option value="lab_section_supervisor">مشرف مجموعة</option>
                <option value="lab_specialist">أخصائي مختبر</option>
              </select>
              {errors.role && (
